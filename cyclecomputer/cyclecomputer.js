    ...
    files.forEach((file) => {
        ...
        if (myArray.length > 1) {
            if (myArray.length > 2) {
                ...
                if (dist > 0 && myArray[2] < dist) {
                    // filter less than distance
                    return;
                }
                if (dist < 0 && myArray[2] > Math.abs(dist)) {
...
export function upArrow() {
    if (ERGMode) {
        ...
        if (trainerDifficulty > 100) {
            trainerDifficulty = 100;
        } // up arrow
    ...
    if (ERGMode) {
        ...
        if (!bWorkout) {
            ...
            if (smartTrainerConnected) {
                ...
                trainerCommands.sendSimulation(
                    grade * 100.0 * (trainerDifficulty / 100.0) + vgear,
                    windSpeed,
                    coefficientRR,
                    coefficientWR
                );
...
export async function processPower(power) {
    ...
    if (appStart) {
        ...
        if (gradeIndex < gpxArray.length - 1 && gradeIndex >= 0) {
            ...
            if (gpsTime !== lastGPSTime) {
                let p1 = new GPXPoint(
                    gpxArray[gradeIndex].lat,
                    gpxArray[gradeIndex].lon,
                    gpxArray[gradeIndex].ele,
                    gpxArray[gradeIndex].smoothEle,
                    gpxArray[gradeIndex].secs,
                    gpxArray[gradeIndex].grade,
                    gpxArray[gradeIndex].smoothGrade,
                    gpxArray[gradeIndex].totaldistancem,
                    gpxArray[gradeIndex].mps,
                    gpsTime,
                    heartRate,
                    power
                );
    ...
    if (appStart) {
        ...
        if (bUseAveVelocity) {
            ...
            console.log(
                "ratio " + ratio.toFixed(1) + "orig mph " + mph.toFixed(1) + " calc mph " + velocity.toFixed(1)
            );
            changeVideoSpeed(ratio, gpxSeconds, totalSeconds);
        }
...
function gradeAutoShift2(nextGrade) {
    if (bUseAutoShift && gradeIndex != lastGradeIndex) {
        ...
        if (nextGrade > currentGrade && diff > 1.0) {
            // uphill
            ...
            console.log("***Shift down " + Math.round(nextGrade - currentGrade) + " vgear " + vgear);
        } else if (currentGrade > nextGrade && diff < -1.0) {
            // downhill
...
function initStartLoc() {
    ...
    console.log(
        "totalDistance " +
            ...
            lapCounter +
            " course dist " +
            gpxArray[gpxArray.length - 1].distance
    );
...
function skipWorkoutInterval() {
    ...
    if (workoutIndex == zwoArray.length) {
        ...
        console.log("Workout time " + workoutTime + " twatts " + document.getElementById("twatts").textContent);
        document.getElementById("alrt").innerHTML =
            "<b>Decrease Power to </b>" +
            document.getElementById("twatts").textContent +
            //+" for "  +zwoArray[workoutIndex].duration + " seconds";
            " for " +
            formatTime(zwoArray[workoutIndex].duration);
        if (workoutIndex > 0 && tpower > Number(zwoArray[workoutIndex - 1].power) * powerFTP) {
            document.getElementById("alrt").innerHTML =
                "<b>Increase Power to </b>" +
                document.getElementById("twatts").textContent +
                //   +" for " + zwoArray[workoutIndex].duration + " seconds";
                " for " +
                formatTime(zwoArray[workoutIndex].duration);
...
function saveData() {
    ...
    let text =
        ...
        document.getElementById("hrcal").textContent +
        ",NP," +
        Number(normalizedPower.NP).toFixed(0) +
        ",IF," +
        Number(normalizedPower.intensityFactor).toFixed(2) +
        ",TSS," +
        Number(normalizedPower.calcTSS(totalSeconds)).toFixed(0);
